...
export function createCard(item) {
    ...
    // Marca dados de gêneros no card para filtragem (ex: data-genres="acao drama")
    if (item.genres && Array.isArray(item.genres)) {
        // canonicalize genres (lowercase, trim, remove diacritics) so filtering is consistent
            const canonical = item.genres.map(g => g.toString().trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')).join(' ');
        card.dataset.genres = canonical;
    } else {
        card.dataset.genres = '';
    }
